#![allow(dead_code)]
use crate::types::{Error, Note, Project};
use log::error;
...
impl ProjectAccess {
    ...
    pub fn get_option(&mut self, key: &str) -> Result<Option<(String, String)>> {
    ...
    fn internal_get_option(
        &mut self,
        key: &str,
    ) -> Result<Option<(String, String)>, rusqlite::Error> {
        let mut stmt = self
            .conn
            .prepare("SELECT key, value FROM Options WHERE key = ?")?;
    ...
    fn internal_add_project(&mut self, project: &mut Project) -> Result<(), rusqlite::Error> {
        ...
        for (key, val) in project.options.iter_mut() {
            tx.execute(
                "INSERT INTO ProjectOptions (project_id, key, value) VALUES ((SELECT project_id 
				FROM Projects WHERE name = ?), ?, ?)",
                params![project.name, *key, *val],
            )?;
        }
    ...
    fn internal_update_project(&mut self, project: &mut Project) -> Result<(), rusqlite::Error> {
        ...
        tx.execute(
            "DELETE FROM ProjectOptions WHERE project_id = (SELECT project_id FROM
			Projects WHERE name = ?)",
            params![project.name],
        )?;
        ...
        tx.execute(
            ...
            params![project.path.to_str().unwrap(), project.name],
        )?;
    ...
    fn internal_list_projects(&mut self) -> Result<Vec<Project>, rusqlite::Error> {
        ...
        for project in projects.iter_mut() {
            let mut stmt = self.conn.prepare(
                "SELECT key, value FROM ProjectOptions WHERE project_id = 
    ...
    fn internal_get_project(&mut self, name: &str) -> Result<Option<Project>, rusqlite::Error> {
        ...
        let mut stmt = self
            .conn
            .prepare("SELECT name, path FROM Projects WHERE name=?")?;
    ...
    fn internal_forget_project(&mut self, project: &Project) -> Result<(), rusqlite::Error> {
        ...
        tx.execute(
            "DELETE FROM Notes WHERE project_id = (SELECT project_id FROM Projects WHERE name = ?)",
            params![project.name],
        )?;
    ...
    fn internal_create_note(
        &mut self,
        note: &Note,
        project: &Project,
    ) -> Result<(), rusqlite::Error> {
        self.conn.execute(
            "INSERT INTO Notes (project_id, created_date, content) VALUES 
			((SELECT project_id FROM Projects WHERE name = ?), ?, ?)",
    ...
    fn internal_update_note(
        &mut self,
        note: &Note,
        project: &Project,
    ) -> Result<(), rusqlite::Error> {
        self.conn.execute(
            "UPDATE Notes SET content = ? WHERE created_date = ? AND project_id = 
			(SELECT project_id FROM Projects WHERE name = ?)",
            params![note.content, note.creation_date, project.name],
        )?;
        Ok(())
    }
    ...
    fn internal_list_notes(&mut self, project: &Project) -> Result<Vec<Note>, rusqlite::Error> {
        let mut stmt = self.conn.prepare(
            "SELECT created_date, content FROM Notes WHERE project_id = 
			(SELECT project_id FROM Projects WHERE name = ?)",
        )?;
        let mut rows = stmt.query(params![project.name])?;
...
fn setup_tables(conn: &rusqlite::Connection) -> Result<(), rusqlite::Error> {
    ...
    // TODO: use a config file to get this data for first insert into the db
    conn.execute(
        "INSERT INTO Options (key, value) VALUES (?, ?)
      ON CONFLICT(key) DO UPDATE SET value=excluded.value",
        params!["ProjectsDir", "/home/ajani/code/"],
    )?;
